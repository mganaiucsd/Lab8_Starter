describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const shadowRt = await prodItem.getProperty('shadowRoot');
    const shadowButton = await shadowRt.$('button');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 1; i < prodItems.length; i++) {
      ...
      const shadowRt = await prodItems[i].getProperty('shadowRoot');
      const shadowButton = await shadowRt.$('button');
      await shadowButton.click();
    }
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const cartArr = await page.evaluate((x) => {
      const localStore = window.localStorage;
      return localStore.cart;
    });
